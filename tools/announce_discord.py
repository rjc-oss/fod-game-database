...
the commit has been pushed, so the link in the message already works. One message per new game:
    ...
    config 0C818D, https://rjc-oss.github.io/fod-game-database/?game=20260918T005007Z-869a14da

The link opens the site showing that game alone and downloads its save (a plain link to the file would
show the JSON in the tab instead: raw.githubusercontent.com serves it as text/plain).
...
def link(game, site=None):
    """Where the message points: the site, showing this game alone, downloading its save."""
    base = site or os.environ.get("FOD_SITE_URL") or SITE_URL
    return "{}?game={}".format(base, urllib.parse.quote(str(game.get("id", "")), safe=""))
